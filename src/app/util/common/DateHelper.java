...
public class DateHelper {
	 ...
	 * @return
	 */
	public static List<String> getMonthStringList(int beforOffset, int afterOffset, String pattern) {
	 ...
	 * 获取周列表
	 * @param year 年份
	 * @param week_index   从第几周开始
	 * @param offset   一共获取几周
	 * @param patten  参数
	 * @return
	 */
	public static List<Map<String, String>> getWeekList(String year,String week_index,int offset,String patten)
	{
		ArrayList<Map<String, String>> list = new ArrayList<Map<String, String>>();
		...
		if(offset>=0)
		{
			...
			{
				begin_date = getLastDayOfWeek(year, String.valueOf(i), patten);
				end_date = getLastDayOfWeek(year, String.valueOf(i), patten);
				Hashtable<String, String> hash = new Hashtable<String, String>();
				String weekIndex = "";
				if(i<10){
				    weekIndex="0"+Integer.toString(i);
				}else{
				    weekIndex=Integer.toString(i);
				}
				hash.put("WEEK_INDEX", weekIndex);
		...
		else
		{
			for(int i=current_index+offset+1;i<=current_index;i++)
			{
				...
				end_date = getLastDayOfWeek(year, String.valueOf(i), patten);
				Hashtable<String, String> hash = new Hashtable<String, String>();
				hash.put("CURRENT_INDEX", Integer.toString(i));
				hash.put("BEGIN_DATE", begin_date);
				hash.put("END_DATE", end_date);
				list.add(hash);
			}
     ...
     * @return
     */
    public static List<Map<String, String>> getWeekList2(int year, int week_index, int offset, String patten) {
        ArrayList<Map<String, String>> list = new ArrayList<Map<String, String>>();
        int current_index = Integer.valueOf(week_index);
        String begin_date = "";
        String end_date = "";
        if(offset >= 0) {
            for(int i = current_index; i < current_index + offset; i++) {
                begin_date = getFirstDayOfWeek(year, i, patten);
                end_date = getLastDayOfWeek(year, i, patten);
                Hashtable<String, String> hash = new Hashtable<String, String>();
                String weekIndex = "";
                if(i < 10) {
                    weekIndex = "0" + Integer.toString(i);
     ...
     * @return
     */
    public static String getFirstDayOfWeek(int year, int week,String patten) {
        Calendar c = Calendar.getInstance();
     ...
     * @return
     */
    public static String getLastDayOfWeek(int year, int week,String patten) {
        Calendar c = Calendar.getInstance();
        c.set(Calendar.YEAR, year);
        c.set(Calendar.WEEK_OF_YEAR, week);
        c.set(Calendar.DAY_OF_WEEK, Calendar.MONDAY);//设置周一
        c.setFirstDayOfWeek(Calendar.MONDAY);
        c.add(Calendar.DAY_OF_MONTH, 6);//6天后
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        System.out.println(getWeekList("2015","6",6,"yyyyMMdd"));
    }
	 ...
	 * @return
	 */
	public static int getWeekIndexOfToday(){
	    Calendar cal=Calendar.getInstance();
